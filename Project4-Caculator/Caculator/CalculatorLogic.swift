...
class CalculatorLogic {
    ...
    //临时保存MainLabel的内容，当其值为true时，输入数组时MainLabel的内容清零
    ...
    func updateMainLabelStringByNumberTag(_ tag: Int, withMainLabelString mainLabelString: String) -> String {
        ...
        //判读主标签的内容转换为double类型后是否等于0， doesStringContainDecimal方法判断是否有小数点
    ...
    //计算方法
    func calculatorByTag(_ tag : Int, withMainLabelString maiLabelString : String) -> String {
        //把string转换为double
        let currentValue = (maiLabelString as NSString).doubleValue
        ...
        let resultSting = NSString(format: "%g", lastRetainValue)
        
        isMainLabelTextTemporary = true
        
        return resultSting as String
        
    }
    
    //清楚方法， 将calculatorLogic类恢复到初始状态后，可以重新实例化calculatorLogic类来达到这个目的，但是重新实例化会耗费过多资源
    //此方法通过初始化存储属性来达到目的
    func clean() {
        lastRetainValue = 0.0
        isMainLabelTextTemporary = false
